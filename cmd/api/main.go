...
import (
	...
	"github.com/raymond-design/task/api/repos"
...
func main() {
	flag.Parse()
	db := repos.NewDB(*dbFlag, *dbCfgFlag)
	repo := repos.NewReminders(db)
	service := services.NewReminders(repo)
	backend := api.New(*addrFlag, service)
	...
	api.ListenForSignals(signals, backend, saver, notifier, db)
}
